...
func (e *EtcdBuilder) Build(appid string) naming.Resolver {
	...
	app.once.Do(func() {
		go app.watch(appid)
		log.Infof("etcd: AddWatch(%s) already watch(%v)", appid, ok)
	})
	return r
}
...
func (e *EtcdBuilder) unregister(ins *naming.Instance) (err error) {
	...
	if _, err = e.cli.Delete(context.TODO(), prefix); err != nil {
		log.Errorf("etcd: unregister client.Delete(%v) appid(%s) hostname(%s) error(%v)",
			prefix, ins.AppID, ins.Hostname, err)
	}
	log.Infof("etcd: unregister client.Delete(%v)  appid(%s) hostname(%s) success",
