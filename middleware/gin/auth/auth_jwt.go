...
type GinJWTMiddleware struct {
	...
	// AfterAuthorized 认证之后
	AfterAuthorized func(c *gin.Context, data jwt.MapClaims)
...
var (
	...
	// ErrNoTokenInRequest can't find token
...
func (mw *GinJWTMiddleware) MiddlewareInit() error {
	...
	if len(mw.TokenLookup) == 0 {
		mw.TokenLookup = append(mw.TokenLookup, "header:Authorization")
	}
...
func (mw *GinJWTMiddleware) middlewareImpl(c *gin.Context) {
	...
	oaToken, _ := c.Get("oa_token")
	checkloginUrl, _ := c.Get("checklogin_url")
	bid, _ := c.Get("bid")
	if oaToken != nil && checkloginUrl != nil && bid != nil {
		if err := checkLogin(oaToken.(string), checkloginUrl.(string)); err != nil {
			mw.unauthorized(c, http.StatusUnauthorized, mw.HTTPStatusMessageFunc(err, c))
			return
		}
	}
...
func (mw *GinJWTMiddleware) TokenGenerator(c *gin.Context, data jwt.MapClaims) (string, time.Time, error) {
...
func (mw *GinJWTMiddleware) extractToken(c *gin.Context) (string, error) {
	var token string
	var err error
	for _, tokenLookup := range mw.TokenLookup {
